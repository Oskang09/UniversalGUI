...
public abstract class ShopRequirement
{
	private String name;
	...
	public void loadConfigRecursive(FileConfiguration cs, String slot, String key, String lopkey)
	{
		for (String key2 : cs.getConfigurationSection(lopkey).getKeys(false))
		{
			...
			{
				this.configmap.put((lopkey + "." + key2).replaceAll("event." + slot + ".requirement." + key + "." + this.getName() + ".", ""), cs.get(lopkey + "." + key2));
			}
		}
	}
	...
	{
		for (String key2 : fcfg.getConfigurationSection("event." + slot + ".requirement." + key).getKeys(false))
		{
			if (key2.equals(name))
			{
				return true;
